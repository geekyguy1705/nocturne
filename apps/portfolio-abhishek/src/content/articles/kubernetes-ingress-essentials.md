...
An Ingress is a Kubernetes object that defines how traffic should be routed to your services. However, it does nothing on its own ΓÇö it needs an Ingress Controller to fulfill the routing rules defined in the Ingress resource.
...
> Be careful when testing locally or using self-signed certs ΓÇö this can cause redirect loops or unexpected failures if TLS isn't set up correctly.
...
8. Watch Resource Limits
   - Ingress controllers run as pods ΓÇö make sure they have CPU/memory limits.
